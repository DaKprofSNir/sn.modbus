...
char BinToHex( data,quartet )      Conversion Binaire->HEXA
...
/*-------------------------------------------
   ...
   sachant qu'une trame ne dépasse jamais 2024 bits utiles.
-------------------------------------------*/
...
//fonctions de convertion bin->hexa. position du quartet = 3, 2, 1, ou 0
char BinToHex( uint16_t w, byte quartet ){
    switch(quartet){
        case 3: w>>=4; 
        case 2: w>>=4;
        case 1: w>>=4;
        case 0: w&=0x000Fu; 
        return (w>0x0009U) ? w+'A'-10:w+'0';
    }//drole de witch
    return '-'; //si quartet hors limite
}//END BinToHex(uint16_t)

char BinToHex( uint8_t b, uint8_t quartet ){
    switch(quartet){
        case 1: b>>=4;
        case 0: b&=0x0F; 
        return (b>0x09U) ? b+'A'-10:b+'0';
    }//drole de witch
    return '-'; //si quartet hors limite
}//END BinToHex(uint8_t)
